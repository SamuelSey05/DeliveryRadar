...
function getScriptInitOptions() {
  const options = {};
  const scriptTag = document.querySelector(index.SELECTORS.script);
  if (scriptTag) {
    const params = index.querystring.parse(scriptTag.src.split('?')[1]);
    Object.entries(params).forEach(([key, value]) => {
      if (index.ScriptInitParameters.includes(key)) {
        switch (key) {
          case index.ScriptInitParameter.KEY:
          case index.ScriptInitParameter.BASEURL:
          case index.ScriptInitParameter.CALLBACK:
            options[key] = value;
            break;
          case index.ScriptInitParameter.HEADERS:
            if (value.length > 0)
...
class Utilisation {
  ...
  async send(session) {
    const { key, baseUrl, headers, data, transport = index.dist.fetchTransport(), type, } = session;
...
exports.getScriptInitOptions = getScriptInitOptions;
exports.isEmpty = isEmpty;
exports.parseCoordinates = parseCoordinates;
exports.utilisation = utilisation;
